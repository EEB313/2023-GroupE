...
# We want to look at observations grouped by whether they were taken before aquaculture began (before 2003), during the aquaculture experiment (2003-2007), and after the aquaculture experiment (post 2007)
...
# We had very few data points for after 2007, and even less data points when you divide it by water parameter, so it was best to remove and only compare "before" and "during" values
# We also removed NA (which was sodium) from our characteristics. The data was saved as NA which made it difficult to run some statistical analyses and we were not particularly interested in this variable to begin with.
...
#In most cases the AICc for the Lmm was better or equal to the anova so we used the Lmm for the main variables. Our data is not independent or normally distributed so we cannot use anova.
...
for (i in 1:(length(characteristic.names)-1)){
  #Here and for loops below, we removed TSS from the characteristic names. This was because the variable had only one value therefore it was hard to run in an statistical test. 
...
#Final Function combines the two to run permutation shuffling 1000 times and return the null plot with real value intercept. 
